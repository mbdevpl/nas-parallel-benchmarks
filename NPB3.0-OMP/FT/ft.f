...
c---------------------------------------------------------------------
      ...
      call setup()
      call init_ui(u0, u1, indexmap, dims)
      call compute_indexmap(indexmap, dims)
      call compute_initial_conditions(u1, dims)
      call fft_init (dims)
      call fft(1, u1, u0)
...
c---------------------------------------------------------------------
      ...
      do iter = 1, niter
         ...
         call evolve(u0, u1, iter, indexmap, dims)
...
c---------------------------------------------------------------------
      ...
      subroutine init_ui(u0, u1, indexmap, d)
...
c---------------------------------------------------------------------
      ...
      double complex u1(d(1)+1,d(2),d(3))
      integer  indexmap(d(1)+1,d(2),d(3))
      integer i, j, k
...
c---------------------------------------------------------------------
 ...
 1000 format(//,' NAS Parallel Benchmarks (NPB3.0-OMP)',
     >          ' - FT Benchmark', /)
 1001 format(' Size                : ', i3, 'x', i3, 'x', i3)
 1002 format(' Iterations          :     ', i7)
 1003 format(' Number of active threads :', i7, /)
...
c---------------------------------------------------------------------
      ...
      if (dir .eq. 1) then
         call cffts1(1, dims, x1, x1, y1, y2)
         call cffts2(1, dims, x1, x1, y1, y2)
         call cffts3(1, dims, x1, x2, y1, y2)
      else
         call cffts3(-1, dims, x1, x1, y1, y2)
         call cffts2(-1, dims, x1, x1, y1, y2)
         call cffts1(-1, dims, x1, x2, y1, y2)
      endif
      return
      end
...
c---------------------------------------------------------------------
      ...
      subroutine cffts1(is, d, x, xout, y1, y2)
...
c---------------------------------------------------------------------
      ...
      double complex x(d(1)+1,d(2),d(3))
      double complex xout(d(1)+1,d(2),d(3))
      double complex y1(fftblockpad, d(1)), y2(fftblockpad, d(1))
      integer i, j, k, jj
...
c---------------------------------------------------------------------
      ...
      subroutine cffts2(is, d, x, xout, y1, y2)
...
c---------------------------------------------------------------------
      ...
      double complex x(d(1)+1,d(2),d(3))
      double complex xout(d(1)+1,d(2),d(3))
      double complex y1(fftblockpad, d(2)), y2(fftblockpad, d(2))
      integer i, j, k, ii
...
c---------------------------------------------------------------------
      ...
      subroutine cffts3(is, d, x, xout, y1, y2)
...
c---------------------------------------------------------------------
      ...
      double complex x(d(1)+1,d(2),d(3))
      double complex xout(d(1)+1,d(2),d(3))
      double complex y1(fftblockpad, d(3)), y2(fftblockpad, d(3))
      integer i, j, k, ii
