...
c---------------------------------------------------------------------------c
      ...
      integer ierr, fstatus, log_p, color
     ...
     >            ' totcomp', ' totcomm'/
      ...
      call mpi_comm_size(mpi_comm_world, nprocs_total, ierr)
...
c---------------------------------------------------------------------
c Determine active set of processes
c---------------------------------------------------------------------
      log_p  = log(float(nprocs_total)+0.0001)/log(2.0)
      nprocs = 2**log_p
 ...
 20      format(' ERROR: requested for ',i8,' processes'//
     &          ' The maximum size allowed for this benchmark is ',i6)
      ...
      call mpi_comm_split(mpi_comm_world,color,me,comm_work,ierr)
      if (color .eq. 1) go to 999
...
c---------------------------------------------------------------------
 ...
 1004    format(' WARNING: The benchmark requires power-of-two',
     >          ' processes.'/' Number of active processes: ', i6)
         if (nprocs .ne. nprocs_total)
     >      write (*, 1004) nprocs
