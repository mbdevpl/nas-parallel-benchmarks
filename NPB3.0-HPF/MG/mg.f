...
!-------------------------------------------------------------------------!
!                                                                         !
!    This benchmark is an HPF version of the NPB MG code.                 !
!                                                                         !
...
c---------------------------------------------------------------------------c
      ...
      integer pad,nx_max,ny_max,nz_max
      parameter (pad=nx_default+2+nx_default/256)
      parameter (nx_max=nx_default+pad)
...
c---------------------------------------------------------------------
      ...
      u(1:n1,1:n2,1:n3,lt) = 0.0
      call zran3(v(:,:,:,lt),n1,n2,n3,nx(lt),ny(lt),k)

      rnm2 = sqrt( sum( v(2:n1-1,2:n2-1,2:n3-1,lt)**2)/
     >                      float( nx(lt)*ny(lt)*nz(lt) ))
c     write(*,*)
...
c     write(*,*)' about to evaluate resid, k=',k
 ...
 1002 format(' Iterations: ', i3)
      ...
      rnm2 = sqrt( sum( r(2:n1-1,2:n2-1,2:n3-1,lt)**2)/
     >                      float( nx(lt)*ny(lt)*nz(lt) ))
      old2 = rnm2
      oldu = rnmu
...
c---------------------------------------------------------------------
      ...
      call timer_start(T_resid2)
      call resid(u(:,:,:,lt),v(:,:,:,lt),r(:,:,:,lt),n1,n2,n3,a,k)
      call timer_stop(T_resid2)
      rnm2 = sqrt( sum( r(2:n1-1,2:n2-1,2:n3-1,lt)**2)/
     >                      float( nx(lt)*ny(lt)*nz(lt) ))
      old2 = rnm2
      oldu = rnmu
...
c---------------------------------------------------------------------
      ...
      call timer_start(T_rprj3)
      do  k= lt, lb+1 , -1
         j = k-1
         call rprj3(r(1:m1(k),1:m2(k),1:m3(k),k),m1(k),m2(k),m3(k),
     >              r(1:m1(j),1:m2(j),1:m3(j),j),m1(j),m2(j),m3(j),k)
      enddo
      call timer_stop(T_rprj3)

      k = lb
c---------------------------------------------------------------------
c     compute an approximate solution on the coarsest grid
c---------------------------------------------------------------------
      u(1:m1(k),1:m2(k),1:m3(k),k) = 0.0
      call timer_start(T_psinv)
      call psinv(r(1:m1(k),1:m2(k),1:m3(k),k),
...
c---------------------------------------------------------------------
         u(1:m1(k),1:m2(k),1:m3(k),k) = 0.0
         call timer_start(T_interp)
         call interp(u(1:m1(j),1:m2(j),1:m3(j),j),m1(j),m2(j),m3(j),
     >               u(1:m1(k),1:m2(k),1:m3(k),k),m1(k),m2(k),m3(k),k)
         call timer_stop(T_interp)
c---------------------------------------------------------------------
c        compute residual for level k
c---------------------------------------------------------------------
      call timer_start(T_resid)
         call resid(u(1:m1(k),1:m2(k),1:m3(k),k),
     >              r(1:m1(k),1:m2(k),1:m3(k),k),
     >              r(1:m1(k),1:m2(k),1:m3(k),k),
     >              m1(k),m2(k),m3(k),a,k)
      call timer_stop(T_resid)
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
 ...
 200  continue
      ...
      call timer_start(T_interp)
      call interp(u(1:m1(j),1:m2(j),1:m3(j),j),m1(j),m2(j),m3(j),
     >            u(1:m1(k),1:m2(k),1:m3(k),k),n1,n2,n3,k)
      call timer_stop(T_interp)
...
c---------------------------------------------------------------------
      ...
      extrinsic (HPF) subroutine rprj3(r,m1k,m2k,m3k,s,m1j,m2j,m3j,k)
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w100(i1,i2,i3) = r(2*i1-1,2*i2,2*i3)
     >                          + r(2*i1-1,2*i2+2,2*i3)
               end do
            end do
         end do
!hpf$ independent, on home(w010(i1,i2,i3))
         do  i3=1,m3j-1
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w010(i1,i2,i3) = r(2*i1,2*i2-1,2*i3)
     >                          + r(2*i1+2,2*i2-1,2*i3)
               end do
            end do
         end do
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w110(i1,i2,i3) = r(2*i1-1,2*i2-1,2*i3)
               end do
            end do
         end do
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w001(i1,i2,i3) = r(2*i1,2*i2,2*i3-1)
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w101(i1,i2,i3) = r(2*i1-1,2*i2,2*i3-1)
               end do
            end do
         end do
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w011(i1,i2,i3) = r(2*i1,2*i2-1,2*i3-1)
               end do
            end do
         end do
...
!hpf$ independent
            do  i2=1,m2j-1
               do i1=1,m1j-1
                 w111(i1,i2,i3) = r(2*i1-1,2*i2-1,2*i3-1)
               end do
            end do
         end do
     ...
     >                  +  w101(2:m1j-1,1:m2j-2,2:m3j-1)
     >                  +  w101(2:m1j-1,2:m2j-1,2:m3j-1)
     >                  +      w110(2:m1j-1,2:m2j-1,1:m3j-2)
     >                  +      w110(2:m1j-1,2:m2j-1,2:m3j-1) )
     >         + 0.125D0 * ( w001(1:m1j-2,1:m2j-2,2:m3j-1)
     >                  +    w001(2:m1j-1,1:m2j-2,2:m3j-1)
...
c      parameter( m=535 )
      ...
      double precision w000(mm1,mm2,mm3), w100(mm1,mm2,mm3),
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
         u(2*i1,2*i2,2*i3) = u(2*i1,2*i2,2*i3) + w000(i1,i2,i3)
     >                     + w000(i1+1,i2,i3)
               end do
            end do
         end do         
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
       u(2*i1-1,2*i2,2*i3) = u(2*i1-1,2*i2,2*i3)+w100(i1,i2,i3)
               end do
            end do
         end do
!hpf$ independent, on home(w010(i1,i2,i3))
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
       u(2*i1,2*i2-1,2*i3) = u(2*i1,2*i2-1,2*i3)+w010(i1,i2,i3)
     >                     + w010(i1+1,i2,i3)
               end do
            end do
         end do
...
!hpf$ independent
           do  i2=1,mm2-1
               do i1=1,mm1-1
         u(2*i1,2*i2,2*i3-1) = u(2*i1,2*i2,2*i3-1) + w001(i1,i2,i3)
     >                     + w001(i1+1,i2,i3)
               end do
            end do
         end do         
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
       u(2*i1-1,2*i2,2*i3-1) = u(2*i1-1,2*i2,2*i3-1)+w101(i1,i2,i3)
               end do
            end do
         end do
!hpf$ independent, on home(w011(i1,i2,i3))
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
       u(2*i1,2*i2-1,2*i3-1) = u(2*i1,2*i2-1,2*i3-1)+w011(i1,i2,i3)
               end do
            end do
         end do
!hpf$ independent, on home(w111(i1,i2,i3))
...
!hpf$ independent
            do  i2=1,mm2-1
               do i1=1,mm1-1
       u(2*i1-1,2*i2-1,2*i3-1) = u(2*i1-1,2*i2-1,2*i3-1)+w111(i1,i2,i3)
               end do
            end do
         end do
...
c---------------------------------------------------------------------
      ...
      extrinsic (HPF) subroutine rep_nrm(u,n1,n2,n3,title,kk)
...
c!hpf$    align(*,*,:) with grid :: u
      ...
      rnm2 = sqrt( sum( u(2:n1-1,2:n2-1,2:n3-1)**2)/
