...
c---------------------------------------------------------------------
       ...
       integer i, ii, j, k, n, k1, k2, m
       double precision ru1, fac1, fac2, rtmp(5,0:KMAX-1)
...
c---------------------------------------------------------------------
       ...
       rhsz(:,1:nx2,1:ny2,0:nz2+1) = rhs(:,1:nx2,1:ny2,0:nz2+1)
       wsz   (1:nx2,1:ny2,0:nz2+1) = ws   (1:nx2,1:ny2,0:nz2+1)
       rho_iz(1:nx2,1:ny2,0:nz2+1) = rho_i(1:nx2,1:ny2,0:nz2+1)
       speedz(1:nx2,1:ny2,0:nz2+1) = speed(1:nx2,1:ny2,0:nz2+1)
       if (timeron) call timer_stop(t_rdis1)
        
!hpf$ independent, new(ru1,cv,rhos,fac1,fac2,rtmp,lhs,lhsp,lhsm)
       do   j = 1, ny2
          do   i = 1, nx2
...
c---------------------------------------------------------------------
     ...
     >                          dz1)
        ...
        do ii = 0, grid_points(3)-1, grid_points(3)-1
          do   n = 1, 5
             lhs (n,ii) = 0.0d0
             lhsp(n,ii) = 0.0d0
             lhsm(n,ii) = 0.0d0
...
c---------------------------------------------------------------------
             ...
             do   k = 0, nz2+1
	        rtmp(1,k) = rhsz(1,i,j,k)
	        rtmp(2,k) = rhsz(2,i,j,k)
	        rtmp(3,k) = rhsz(3,i,j,k)
	        rtmp(4,k) = rhsz(4,i,j,k)
	        rtmp(5,k) = rhsz(5,i,j,k)
	     end do
...
c---------------------------------------------------------------------
             do   k = 0, nz2+1
	        rhsz(1,i,j,k) = rtmp(1,k)
	        rhsz(2,i,j,k) = rtmp(2,k)
	        rhsz(3,i,j,k) = rtmp(3,k)
	        rhsz(4,i,j,k) = rtmp(4,k)
	        rhsz(5,i,j,k) = rtmp(5,k)
	     end do

          end do
       end do
       if (timeron) call timer_start(t_rdis2)
       rhs(:,1:nx2,1:ny2,0:nz2+1) = rhsz(:,1:nx2,1:ny2,0:nz2+1)
